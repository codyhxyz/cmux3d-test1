...
export function readCloudOptions(env = process.env, argv = process.argv, { force = false } = {}) {
  const { values } = parseNodeArgs({
    ...
    options: Object.fromEntries(
      ['runtime-arn', 'session', 'region', 'qualifier', 'expires', 'signer', 'origin']
        .map((name) => [name, { type: 'string', multiple: true }]),
    ),
  });
  // The old CLI contract is first-value-wins, including repeated --origin. Keep that
  // while delegating spaced/inline tokenization to Node.
  const value = (name) => {
    const first = values[name]?.[0];
    return typeof first === 'string' && !first.startsWith('--') ? first : '';
  };
  const runtimeArn = value('runtime-arn') || env.CUBE_RUNTIME_ARN || '';
  const disabled = env.CODING_CUBE_CLOUD === '0' || argv.includes('--no-cloud');
  if (!runtimeArn || (disabled && !force)) return null;
  const session = value('session') || env.CUBE_SESSION_ID || '';
  ...
  return {
    ...
    region: value('region') || env.CUBE_REGION || runtimeArn.split(':')[3] || env.AWS_REGION || 'us-east-1',
    qualifier: value('qualifier') || env.CUBE_QUALIFIER || 'DEFAULT',
    sessionId: session || null,
    pinSession: argv.includes('--pin-session') || env.CUBE_PIN_SESSION === '1',
    expiresIn: Number(value('expires') || env.CUBE_MINT_EXPIRES || 300),
    signer: value('signer') || env.CUBE_SIGNER || 'aws-sdk',
    profile,
    ...
    origins: String(value('origin') || env.CUBE_MINT_ORIGINS || '')
      .split(',')
